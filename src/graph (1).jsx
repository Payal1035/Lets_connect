...
class BarGraph extends React.Component {
	render() {
		...
		values = React.Children.map(this.props.children, item => item.props.value),
		colors = React.Children.map(this.props.children, (graphItem, i) => graphItem.props.color || `hsl(${i / values.length * 360}, 66%, 50%)`),
		...
		let x = 0;
		const rects = React.Children.map(this.props.children, (item, i) => {
...
class PieChart extends React.Component {
	render() {
		...
		const total = React.Children.map(this.props.children, pieItem => pieItem.props.value).reduce((a, b) => a + b, 0);
		const colors = React.Children.map(this.props.children, (pieItem, i) => pieItem.props.color || `hsl(${i / React.Children.count(this.props.children) * 360}, 66%, 50%)`);
		const info = React.Children.map(this.props.children, (pieItem, i) => (
			<div><span style={{color: colors[i]}}>&#9632;</span> {pieItem.props.name}: {pieItem.props.value} ({((pieItem.props.value / total) * 100).toFixed(2)}%)</div>
		));
		const arcs = React.Children.map(this.props.children, (pieItem, i) => {
			startAngle = endAngle;
